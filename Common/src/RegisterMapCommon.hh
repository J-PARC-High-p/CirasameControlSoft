...
namespace MIG{
  enum LocalAddress
    {
     kAddrNumWrite  = 0x20000000, // W/R, [5:0] Number of writes to SDRAM
     kAddrNumRead   = 0x20100000  // W/R, [5:0] Number of reads from SDRAM
    };
};
...
namespace AD9220{
  enum LocalAddress
    {
     kAddrDaqGate     = 0x50000000, // W/R, [0:0] ADC DAQ gate
     kAddrEventFull   = 0x50100000, // R,   [0:0] ADC event fifo is full
     kAddrReadFIFO    = 0x51000000, // R,   FIFO read sequence
     kAddrReleaseBusy = 0x52000000  // W,   Release busy (event full)
    };
};
...
namespace HGD{
  enum LocalAddress
    {
     kAddrMask        = 0x60000000  // W/R, [31:0] Delay line mask
    };
};
};
