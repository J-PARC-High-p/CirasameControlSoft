...
int main(int argc, char* argv[])
{
  ...
  RBCP::UDPRBCP udp_rbcp(kBoardIp, RBCP::gUdpPort, RBCP::UDPRBCP::kNoDisp);
  HUL::FPGAModule fpga_module(udp_rbcp);

  fpga_module.WriteModule(CIRASAME::AD9220::kAddrDaqGate, 0);
  fpga_module.WriteModule(CIRASAME::AD9220::kAddrDaqGate, 1);
  ...
  for(int i = 0; i<kEventNum; ++i){
    while(0 == fpga_module.ReadModule(CIRASAME::AD9220::kAddrEventFull, 1));

    for(int i = 0; i<2; ++i){
      fpga_module.ReadModule_nByte(CIRASAME::AD9220::kAddrReadFIFO, kReadLength/2);
    ...
    fpga_module.WriteModule(CIRASAME::AD9220::kAddrReleaseBusy, 0);
