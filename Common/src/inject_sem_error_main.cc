...
int main(int argc, char* argv[])
{
  ...
  printf("Inject SEU through SEM\n");
  set_error_injection_address(fpga_module, SDS::kSetIdle, 0, 0, 0);
  fpga_module.WriteModule(SDS::kAddrSemErrStrobe, 1);

  set_error_injection_address(fpga_module, SDS::kSetError, 100, 10, 1);
  fpga_module.WriteModule(SDS::kAddrSemErrStrobe, 1);

  set_error_injection_address(fpga_module, SDS::kSetObserve, 0, 0, 0);
  fpga_module.WriteModule(SDS::kAddrSemErrStrobe, 1);
